...
**Core principle:** Agent automates everything with CLI/API. Checkpoints are for verification and decisions, not manual work.
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>[What Agent automated and deployed/built]</what-built>
  <how-to-verify>
...
**Key elements:**
- `<what-built>`: What Agent automated (deployed, built, configured)
...
**When:** Action has NO CLI/API and requires human-only interaction, OR Agent hit an authentication gate during automation.

**Use ONLY for:**
- **Authentication gates** - Agent tried to use CLI/API but needs credentials to continue (this is NOT a failure)
...
<task type="checkpoint:human-action" gate="blocking">
  <action>[What human must do - Agent already did everything automatable]</action>
  <instructions>
    [What Agent already automated]
    [The ONE thing requiring human action]
  </instructions>
  <verification>[What Agent can check afterward]</verification>
  <resume-signal>[How to continue]</resume-signal>
</task>
```

**Key principle:** Agent automates EVERYTHING possible first, only asks human for the truly unavoidable manual step.
...
**Key distinction:** Authentication gates are created dynamically when Agent encounters auth errors during automation. They're NOT pre-planned - Agent tries to automate first, only asks for credentials when blocked.

See references/cli-automation.md "Authentication Gates" section for more examples and full protocol.

## Execution Protocol

When Agent encounters `type="checkpoint:*"`:
...
**DON'T:**
- Ask human to do work Agent can automate (deploy, create resources, run builds)
- Assume knowledge: "Configure the usual settings" ❌
- Skip steps: "Set up database" ❌ (too vague)
- Mix multiple verifications in one checkpoint (split them)
- Make verification impossible (Agent can't check visual appearance without user confirmation)
...
- Things Agent can verify programmatically (tests pass, build succeeds)
- File operations (Agent can read files to verify)
...
Place checkpoints:
- **After automation completes** - not before Agent does the work
- **After UI buildout** - before declaring phase complete
- **Before dependent work** - decisions before implementation
- **At integration points** - after configuring external services

Bad placement:
- Before Agent automates (asking human to do automatable work) ❌
...
<!-- NO CHECKPOINT NEEDED - Agent automated everything and verified programmatically -->
```
...
**Why bad:** Vercel has a CLI. Agent should run `vercel --yes`.

### ✅ GOOD: Agent automates, human verifies
...
**Why bad:** Agent has Write tool. This should be `type="auto"`.

## Summary

Checkpoints formalize human-in-the-loop points. Use them when Agent cannot complete a task autonomously OR when human verification is required for correctness.

**The golden rule:** If Agent CAN automate it, Agent MUST automate it.

**Checkpoint priority:**
1. **checkpoint:human-verify** (90% of checkpoints) - Agent automated everything, human confirms visual/functional correctness
...
**See also:** references/cli-automation.md for exhaustive list of what Agent can automate.

---
name: create-plans
description: Create hierarchical project plans optimized for solo agentic development. Use when planning projects, phases, or tasks that agent will execute. Produces agent-executable plans with verification criteria, not enterprise documentation. Handles briefs, roadmaps, phase plans, and context handoffs.
---

<essential_principles>

<principle name="solo_developer_plus_agent">
You are planning for ONE person (the user) and ONE implementer (agent).
No teams. No stakeholders. No ceremonies. No coordination overhead.
The user is the visionary/product owner. agent is the builder.
</principle>
...
**Critical insight:** agent doesn't degrade at 80% - it degrades at ~40-50% when it sees context mounting and enters "completion mode." By 80%, quality has already crashed.
...
**agent automates everything that has a CLI or API.** Checkpoints are for verification and decisions, not manual work.

**Checkpoint types:**
- `checkpoint:human-verify` - Human confirms agent's automated work (visual checks, UI verification)
...
**Critical rule:** If agent CAN do it via CLI/API/tool, agent MUST do it. Never ask human to:
...
**Protocol:** agent automates work → reaches checkpoint:human-verify → presents what was done → waits for confirmation → resumes
...
<domain_expertise>
**Domain expertise lives in `~/.agent/skills/expertise/`**
...
| "Agent SDK", "agent SDK", "agentic app" | expertise/with-agent-sdk |
...
5. with-agent-sdk - agent Agent SDK apps
...
```bash
cat ~/.agent/skills/expertise/[domain]/SKILL.md 2>/dev/null
```

...
4.  **Building unlocks**: Ensure each new building expands item pools or automation. For example, the Resin Forge unlocked by rescuing the Carpenter enables resin processing.
    - _Phase 2 deliverable_: data-driven `UnlockRequirement` contracts on `BuildingDefinition` and `ProductionRecipeDefinition`, catalog validation, and read-only `ISettlementUnlockReadModel` filtering in presentation.
    - _Phase 3 handoff_: NPC-specific unlock facts (e.g., Carpenter recruited → Resin Forge available) are owned by NPC/Settlement systems and must be surfaced through typed events or an owner-provided read model that implements `ISettlementUnlockReadModel`. Phase 2 does not implement NPC rescue, recruitment, or temporary unlock workarounds.
5.  **Trading & barter**: Design merchants and factions that exchange items for resources rather than currency.
    - _Research 02-08 boundary_: Full barter runtime is deferred until merchant/faction and item ownership contracts exist. Phase 2 should not add a resource-only barter board or static offer contracts with placeholder merchant/item ids. Future barter must read `Settlement.Contracts` resource ids/amounts and route settlement storage mutations through Settlement-owned request/event handlers.

_Status:_ Resource node generation, harvest profiles, depletion hazards and client visuals now cover trees, ores, spore pods and chests. Building interaction UX, resource overlays and carried-inventory deposit into settlement stockpiles exist. Settlement now has generalized production station contracts, current Workbench recipes/state, server-authoritative recipe processing with fuel requirements and explicit blocked-state reporting, output claiming into shared storage, and data-driven unlock gates for buildings and recipes with catalog validation and read-only presentation filtering. Additional station buildings, bartering and broader economy sinks remain to be implemented.

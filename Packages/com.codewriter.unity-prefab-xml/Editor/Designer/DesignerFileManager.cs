...
namespace UnityPrefabXML.Designer
{
    public static class DesignerFileManager
    {
        ...
        private static void BuildParallelMapping(Transform baseTf, Transform variantTf, XElement goElement,
            DesignerContext ctx)
        {
            ctx.GoToXml[baseTf.gameObject.GetEntityId()] = goElement;
            ctx.GoToVariant[baseTf.gameObject.GetEntityId()] = variantTf;
            ...
            foreach (var baseComp in baseComps)
            {
                ...
                for (var i = 0; i < xmlCompElements.Count; i++)
                {
                    ...
                    {
                        ctx.CompToXml[baseComp.GetEntityId()] = xmlCompElements[i];
            ...
            foreach (var baseComp in baseComps)
            {
                ...
                for (var i = 0; i < variantComps.Count; i++)
                {
                    ...
                    {
                        ctx.CompToVariant[baseComp.GetEntityId()] = variantComps[i];
        ...
        private static void ApplyPropertyModifications(PropertyModification[] modifications, DesignerContext ctx)
        {
            ...
            var modsByTarget = modifications
                ...
                .GroupBy(m => m.target.GetEntityId());

            foreach (var group in modsByTarget)
            {
                var target = modifications.First(m => m.target.GetEntityId() == group.Key).target;
                var mods = group.ToList();

                // Handle GameObject-level modifications (name, active)
                if (target is GameObject targetGo)
                {
                    if (!ctx.GoToXml.TryGetValue(targetGo.GetEntityId(), out var goXml))
                    {
                        continue;
                    }
                ...
                if (!ctx.CompToXml.TryGetValue(targetComp.GetEntityId(), out var xmlElement))
                {
                    continue;
                }

                if (!ctx.CompToVariant.TryGetValue(targetComp.GetEntityId(), out var variantComp))
                {
                    continue;
                }
        ...
        private static void ApplyRemovedComponents(List<RemovedComponent> removedComponents, DesignerContext ctx)
        {
            foreach (var removed in removedComponents)
            {
                ...
                if (ctx.CompToXml.TryGetValue(comp.GetEntityId(), out var xmlElement))
                {
                    xmlElement.Remove();
                }
            }
        ...
        private static void ApplyRemovedGameObjects(List<RemovedGameObject> removedGameObjects, DesignerContext ctx)
        {
            foreach (var removed in removedGameObjects)
            {
                ...
                if (ctx.GoToXml.TryGetValue(go.GetEntityId(), out var xmlElement))
                {
                    xmlElement.Remove();
                }
            }
    ...
    public class DesignerContext
    {
        ...
        public readonly Dictionary<EntityId, XElement> CompToXml = new();
        public readonly Dictionary<EntityId, Component> CompToVariant = new();
        public readonly Dictionary<EntityId, XElement> GoToXml = new();
        public readonly Dictionary<EntityId, Transform> GoToVariant = new();

        // Bindings
        public readonly Dictionary<string, Object> NewBindings = new();
        public readonly HashSet<string> UsedBindingNames = new();

        public EntityId FindBaseGoIdForVariantGo(GameObject variantGo)
        {
        ...
        public EntityId FindBaseGoIdForVariantTf(Transform variantTf)
        {
            foreach (var kvp in GoToVariant)
            {

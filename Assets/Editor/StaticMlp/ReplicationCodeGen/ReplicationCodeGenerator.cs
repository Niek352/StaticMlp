...
namespace StaticMlp.Editor.ReplicationCodeGen {
    ...
    public sealed class ReplicationCodeGenerator : ICodeGenerator {
        ...
        public void Execute(GeneratorContext context) {
            ...
            context.AddCode("ReplicatedComponentRegistry.Generated.cs", EmitRegistry(components, networkEntities));
        }
        ...
        private static string EmitRegistry(List<ComponentInfo> components, List<NetworkEntityInfo> networkEntities) {
            ...
            AppendNetworkEntityMetadata(builder, networkEntities);
            builder.AppendLine();
            ...
            builder.AppendLine();
            AppendCollectDirtyByEntityType(builder, "CW", components, networkEntities);
            builder.AppendLine();
            AppendCollectDirtyByEntityType(builder, "SW", components, networkEntities);
            builder.AppendLine();
            AppendCollectClientOwnedDirty(builder);
            builder.AppendLine();
            AppendCollectServerOwnedDirty(builder);
            builder.AppendLine();
            AppendInitialState(builder, components, networkEntities);
            builder.AppendLine();
            AppendRegisterClientCoreGeneratedTypes(builder, components);
            builder.AppendLine();
            AppendRegisterClientCoreInterpolationSystems(builder, components);
            builder.AppendLine();
            AppendInitializeClientCoreInterpolatedState(builder, components);
            foreach (var component in components) {
                builder.AppendLine();
        ...
        private static void AppendApply(StringBuilder builder, string worldAlias, List<ComponentInfo> components) {
            ...
            foreach (var component in components) {
                ...
                if (worldAlias == "CW" && component.HasInterpolatedFields)
                    builder.AppendLine($"                    {component.Type.Name}Replication.ApplyClientDelta(e, delta.Payload);");
                else
        ...
        private static void AppendNetworkEntityMetadata(StringBuilder builder, List<NetworkEntityInfo> networkEntities) {
            builder.AppendLine("        public static ushort GetNetworkSchemaVersion(byte entityType) {");
            builder.AppendLine("            return entityType switch {");
            foreach (var entity in networkEntities)
                builder.AppendLine($"                {entity.EntityTypeId} => {entity.NetworkSchemaVersion},");
            builder.AppendLine("                _ => 0");
            builder.AppendLine("            };");
            builder.AppendLine("        }");
            builder.AppendLine();
            builder.AppendLine("        public static ushort GetDefaultNetworkArchetypeId(byte entityType) {");
            builder.AppendLine("            return entityType switch {");
            foreach (var entity in networkEntities)
                builder.AppendLine($"                {entity.EntityTypeId} => {entity.DefaultNetworkArchetypeId},");
            builder.AppendLine("                _ => 0");
            builder.AppendLine("            };");
            builder.AppendLine("        }");
        }
        ...
        private static void AppendCollectClientOwnedDirty(StringBuilder builder) {
            builder.AppendLine("        public static void CollectClientOwnedDirty(NetOutbox outbox, NetworkPeerId peer) {");
            builder.AppendLine("            foreach (var e in CW.Query<All<LocalOwned, NetworkedTag, NetworkIdentity, NetworkDirty, NetworkReplicationState>>().Entities()) {");
            builder.AppendLine("                CollectDirtyByEntityType(e, outbox, peer);");
            builder.AppendLine("                e.Delete<NetworkDirty>();");
            builder.AppendLine("            }");
            builder.AppendLine("        }");
        }

        private static void AppendCollectServerOwnedDirty(StringBuilder builder) {
            builder.AppendLine("        public static void CollectServerOwnedDirty(NetOutbox outbox, IReadOnlyList<NetworkPeerId> peers) {");
            builder.AppendLine("            foreach (var e in SW.Query<All<ServerOwned, NetworkedTag, NetworkIdentity, NetworkDirty, NetworkReplicationState>>().Entities()) {");
            builder.AppendLine("                CollectDirtyByEntityType(e, outbox, peers);");
            builder.AppendLine("                e.Delete<NetworkDirty>();");
            builder.AppendLine("            }");
            builder.AppendLine("        }");
        }
        ...
        private static void AppendInitialState(StringBuilder builder, List<ComponentInfo> components, List<NetworkEntityInfo> networkEntities) {
            ...
            builder.AppendLine();
            builder.AppendLine("            CollectInitialStateByEntityType(e, components);");
            builder.AppendLine("        }");
            builder.AppendLine();
            builder.AppendLine("        private static void CollectInitialStateByEntityType(SW.Entity e, List<ComponentDelta> components) {");
            builder.AppendLine("            switch (e.EntityType) {");
            foreach (var entity in networkEntities) {
                builder.AppendLine($"                case {entity.EntityTypeId}:");
                builder.AppendLine($"                    Collect{entity.Type.Name}InitialState(e, components);");
                builder.AppendLine("                    return;");
            }
            builder.AppendLine("            }");
            builder.AppendLine();
            builder.AppendLine("            if (e.Has<NetworkIdentity>()) {");
            builder.AppendLine("                CollectInitialStateByNetworkArchetype(e.Read<NetworkIdentity>().NetworkArchetypeId, e, components);");
            builder.AppendLine("                return;");
            builder.AppendLine("            }");
            builder.AppendLine();
            builder.AppendLine("            CollectInitialStateFallback(e, components);");
            builder.AppendLine("        }");
            builder.AppendLine();
            builder.AppendLine("        private static void CollectInitialStateByNetworkArchetype(ushort networkArchetypeId, SW.Entity e, List<ComponentDelta> components) {");
            builder.AppendLine("            switch (networkArchetypeId) {");
            foreach (var entity in networkEntities.Where(x => x.DefaultNetworkArchetypeId != 0)) {
                builder.AppendLine($"                case {entity.DefaultNetworkArchetypeId}:");
                builder.AppendLine($"                    Collect{entity.Type.Name}InitialState(e, components);");
            ...
            foreach (var entity in networkEntities) {
                builder.AppendLine();
                builder.AppendLine($"        private static void Collect{entity.Type.Name}InitialState(SW.Entity e, List<ComponentDelta> components) {{");
                foreach (var component in entity.Manifest) {
                    var name = GetTypeName(component.Type);
            ...
            builder.AppendLine();
            builder.AppendLine("        private static void CollectInitialStateFallback(SW.Entity e, List<ComponentDelta> components) {");
            foreach (var component in components) {
                var name = GetTypeName(component.Type);
                builder.AppendLine();
                builder.AppendLine($"            if (e.Has<{name}>())");
        ...
        private static void AppendRegisterClientCoreGeneratedTypes(StringBuilder builder, List<ComponentInfo> components) {
            builder.AppendLine("        public static void RegisterClientCoreGeneratedTypes() {");
            var interpolated = components.Where(x => x.HasInterpolatedFields).ToList();
            if (interpolated.Count != 0) {
                builder.AppendLine("            CW.Types()");
                for (var i = 0; i < interpolated.Count; i++) {
                    var name = GetTypeName(interpolated[i].Type);
                    var terminator = i == interpolated.Count - 1 ? ";" : string.Empty;
                    builder.AppendLine($"                .Component<Interpolated<{name}>>()");
                    builder.AppendLine($"                .Component<InterpolatedPrevious<{name}>>()");
                    builder.AppendLine($"                .Component<InterpolatedClock<{name}>>(){terminator}");
                }
            }
            builder.AppendLine("        }");
        }

        private static void AppendRegisterClientCoreInterpolationSystems(StringBuilder builder, List<ComponentInfo> components) {
            builder.AppendLine("        public static void RegisterClientCoreInterpolationSystems(ClientCoreSystemsBuilder systems) {");
            foreach (var component in components.Where(x => x.HasInterpolatedFields)) {
                var name = GetTypeName(component.Type);
                builder.AppendLine($"            systems.Add(new ReplicatedInterpolationSystem<{name}>({component.Type.Name}Replication.Interpolate), GameplaySystemOrder.ClientPresentation);");
            }
            builder.AppendLine("        }");
        }

        private static void AppendInitializeClientCoreInterpolatedState(StringBuilder builder, List<ComponentInfo> components) {
            builder.AppendLine("        public static void InitializeClientCoreInterpolatedState(CW.Entity e) {");
            foreach (var component in components.Where(x => x.HasInterpolatedFields)) {
                var name = GetTypeName(component.Type);
                builder.AppendLine($"            if (e.Has<{name}>() && !e.Has<Interpolated<{name}>>()) {{");
                builder.AppendLine($"                var state = e.Read<{name}>();");
                builder.AppendLine($"                e.Set(new Interpolated<{name}>(state));");
                builder.AppendLine($"                e.Set(new InterpolatedPrevious<{name}>(state));");
                builder.AppendLine($"                e.Set(new InterpolatedClock<{name}> {{");
                builder.AppendLine("                    StartedAt = Time.time,");
                builder.AppendLine($"                    Duration = {component.Type.Name}Replication.SendRate == 0 ? 0f : 1f / {component.Type.Name}Replication.SendRate");
                builder.AppendLine("                });");
                builder.AppendLine("            }");
            }
            builder.AppendLine("        }");

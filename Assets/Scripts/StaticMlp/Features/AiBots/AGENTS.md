...
- Spawn bots through `InitialBotSpawningResource` and `AiBotSpawns`. Do not hand-assemble bot entities in unrelated systems.
...
- Fail fast when required resources such as `AiActionCatalog`, `AiNavigationRuntime`, or `InitialBotSpawningResource` are missing or null.
...
- For bootstrap and spawn flow, read `AiBotsGameplayFeature`, `AiBotSpawns`, and `InitialBotSpawningResource`.

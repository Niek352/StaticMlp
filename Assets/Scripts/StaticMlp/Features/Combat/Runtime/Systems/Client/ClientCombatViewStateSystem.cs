...
{
    public sealed class ClientCombatViewStateSystem : ISystem
    {
        public void Update()
        {
            foreach (var entity in CW.Query<All<CharacterNetState>>().Entities())
            {
                ...
                if (entity.Has<PoisonStatus>())
                    flags |= StatusVisualFlags.Poison;
                if (entity.Has<BurningStatus>())
                    flags |= StatusVisualFlags.Burning;
                if (entity.Has<OiledStatus>())
                    flags |= StatusVisualFlags.Oiled;

                entity.Set(new CombatViewState
                {

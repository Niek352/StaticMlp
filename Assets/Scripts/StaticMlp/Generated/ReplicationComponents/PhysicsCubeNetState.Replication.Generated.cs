...
namespace StaticMlp.Networking.Replication.Generated {
    public static class PhysicsCubeNetStateReplication {
        ...
        public static PhysicsCubeNetState Read(byte[] payload) {
            if (payload == null || payload.Length == 0)
                return default;

            var reader = new BinaryPackReader(payload, (uint)payload.Length, 0);
            return new PhysicsCubeNetState {
        ...
        public static void ApplyClientDelta(CW.Entity e, byte[] payload) {
            ...
            if (e.Has<PhysicsCubeNetState>())
                e.Set(new InterpolatedPrevious<PhysicsCubeNetState>(e.Read<PhysicsCubeNetState>()));
            else
                e.Set(new InterpolatedPrevious<PhysicsCubeNetState>(next));

            if (!e.Has<Interpolated<PhysicsCubeNetState>>())
                e.Set(new Interpolated<PhysicsCubeNetState>(next));
        ...
        public static void Interpolate(in PhysicsCubeNetState previous, in PhysicsCubeNetState current, ref PhysicsCubeNetState interpolated, float alpha) {

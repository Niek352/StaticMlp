    ...
    public sealed class CampFlowRepairFlowTests
    {
        ...
        public void ServerCompleteConstructionSystem_WhenRepairSiteFinishes_EmitsFlowFactHandledByCampFlowOwner()
        {
            ...
            var anchor = scope.CreateSettlementAnchor(
                ...
                Vector3.zero,
                CampFlowStage.RepairResourcesReady);
            var site = scope.CreateNetworkedConstructionSite(buildWorkDone: 95f);
            var siteGid = site.GID;
            var flowSystem = new ServerCampFlowProgressionSystem();
            flowSystem.Init();
        ...
        public void ServerCompleteConstructionSystem_WhenNonCampCoreFinishes_EmitsGenericFactWithoutRepairFact()
        {
            ...
            Assert.That(anchor.Read<CampFlowProgression>().Stage, Is.EqualTo(CampFlowStage.RepairResourcesReady));
        ...
        [Test]
        public void ServerCampFlowProgressionSystem_WhenAnchorSpawns_AdvancesToRepairObjectiveActive()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
                Vector3.zero,
                CampFlowStage.DamagedCampStart);
            ...
            Assert.That(anchor.Read<CampFlowProgression>().Stage, Is.EqualTo(CampFlowStage.RepairObjectiveActive));
        }

        [Test]
        public void ServerCampFlowAnchorSpawnSystem_SpawnsAnchorWithFullRequiredState()
        {
            using var scope = new CombatTestServerWorldScope();
            SW.SetResource(SettlementSeedManifest.CreateResource());

            new ServerCampFlowAnchorSpawnSystem().Update();
            new ServerInitialConstructionSiteSpawnSystem().Update();

            Assert.That(CampFlowProgressionQuery.TryGetServerAnchor(SettlementAnchorCatalog.HomeCampId, out var anchor), Is.True);
            Assert.That(anchor.Has<CampFlowProgression>(), Is.True);
            Assert.That(anchor.Has<CampFlowViewState>(), Is.True);
            Assert.That(anchor.Has<ProgressionState>(), Is.True);
        ...
        public void ServerSettlementSeedSpawnSystems_ProjectInitialCampAndWorkerToTerrainHeight()
        {
            ...
            var anchor = CampFlowProgressionQuery.GetServerAnchor(SettlementAnchorCatalog.HomeCampId);
        ...
        [Test]
        public void ServerCampFlowProgressionSystem_WhenRepairResourcesAreAvailable_AdvancesToRepairResourcesReady()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
                Vector3.zero,
                CampFlowStage.RepairObjectiveActive);
        ...
        [Test]
        public void ServerCampFlowProgressionSystem_WithoutRepairCompletedEvent_DoesNotAdvanceToCampRepaired()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
        ...
        public void SetSettlementWorkerAssignmentHandler_WhenAssignmentAccepted_CampFlowOwnerAdvancesToWorkerAssigned()
        {
            ...
            Assert.That(anchor.Read<CampFlowProgression>().Stage, Is.EqualTo(CampFlowStage.WorkerAssigned));
        }
        ...
        [Test]
        public void ServerCampFlowProgressionSystem_WhenLoadoutPreparedEventReceived_AdvancesToLoadoutPrepared()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
                Vector3.zero,
                CampFlowStage.WorkbenchOnline);

            var flowSystem = new ServerCampFlowProgressionSystem();
            flowSystem.Init();
            SW.SendEvent(new CampFlowLoadoutPreparedEvent(SettlementAnchorCatalog.HomeCampId));
            flowSystem.Update();
            flowSystem.Destroy();

            Assert.That(anchor.Read<CampFlowProgression>().Stage, Is.EqualTo(CampFlowStage.LoadoutPrepared));
        }
        ...
        [Test]
        public void SettlementHudPresentation_WhenCampIsRepaired_ShowsAssignWorkerAndKeepsLoadoutPreparationClosed()
        {
            using var scope = new CampFlowPresentationClientWorldScope();
            scope.CreateAnchor(stage: CampFlowStage.CampRepaired);
        ...
        {
            using var scope = new CombatTestServerWorldScope();
            var seed = SettlementSeedManifest.CreateResource();
        ...
        [Test]
        public void SettlementHudPresentation_WhenRepairResourcesAreReady_ShowsContinueBuildingHint()
        {
            using var scope = new CampFlowPresentationClientWorldScope();
            scope.CreateAnchor(stage: CampFlowStage.RepairResourcesReady);
            scope.CreateSharedResources();
            scope.RefreshProjections();
        ...
        [Test]
        public void ServerCampFlowProgressionSystem_WhenStockpilePlacedEventReceived_AdvancesFromWorkerAssigned()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
            ...
            var flowSystem = new ServerCampFlowProgressionSystem();
            flowSystem.Init();
            SW.SendEvent(new CampFlowStockpilePlacedEvent(SettlementAnchorCatalog.HomeCampId));
            flowSystem.Update();
            flowSystem.Destroy();

            Assert.That(anchor.Read<CampFlowProgression>().Stage, Is.EqualTo(CampFlowStage.StockpilePlaced));
        }

        [Test]
        public void ServerCampFlowProgressionSystem_WhenShelterPlacedEventReceived_AdvancesFromStockpilePlaced()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
                Vector3.zero,
                CampFlowStage.StockpilePlaced);

            var flowSystem = new ServerCampFlowProgressionSystem();
            flowSystem.Init();
            SW.SendEvent(new CampFlowShelterPlacedEvent(SettlementAnchorCatalog.HomeCampId));
            flowSystem.Update();
            flowSystem.Destroy();

            Assert.That(anchor.Read<CampFlowProgression>().Stage, Is.EqualTo(CampFlowStage.ShelterPlaced));
        }

        [Test]
        public void ServerCampFlowProgressionSystem_WhenExtractionOnlineEventReceived_AdvancesFromShelterPlaced()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
                Vector3.zero,
                CampFlowStage.ShelterPlaced);

            var flowSystem = new ServerCampFlowProgressionSystem();
            flowSystem.Init();
            SW.SendEvent(new CampFlowExtractionOnlineEvent(SettlementAnchorCatalog.HomeCampId));
            flowSystem.Update();
            flowSystem.Destroy();

            Assert.That(anchor.Read<CampFlowProgression>().Stage, Is.EqualTo(CampFlowStage.ExtractionOnline));
        }

        [Test]
        public void ServerCampFlowProgressionSystem_WhenWorkbenchOnlineEventReceived_AdvancesFromExtractionOnline()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
                Vector3.zero,
                CampFlowStage.ExtractionOnline);

            var flowSystem = new ServerCampFlowProgressionSystem();
            flowSystem.Init();
            SW.SendEvent(new CampFlowWorkbenchOnlineEvent(SettlementAnchorCatalog.HomeCampId));
            flowSystem.Update();
            flowSystem.Destroy();

            Assert.That(anchor.Read<CampFlowProgression>().Stage, Is.EqualTo(CampFlowStage.WorkbenchOnline));
        }

        [Test]
        public void ServerCampFlowProgressionSystem_WhenStockpilePlacedReceivedAtWrongStage_DoesNotAdvance()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
        ...
        [Test]
        public void ServerCampFlowProgressionSystem_WhenLoadoutPreparedReceivedAtWorkerAssigned_DoesNotAdvance()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
        ...
        [Test]
        public void ServerCampFlowProgressionSystem_WhenExtractionOnlineReceivedAtWorkerAssigned_DoesNotAdvance()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,
        ...
        [Test]
        public void ServerCampFlowProgressionSystem_WhenWorkbenchOnlineReceivedAtStockpilePlaced_DoesNotAdvance()
        {
            using var scope = new CombatTestServerWorldScope();
            var anchor = scope.CreateSettlementAnchor(
                SettlementAnchorCatalog.HomeCampId,

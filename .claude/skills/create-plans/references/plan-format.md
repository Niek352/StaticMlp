<overview>
Agent-executable plans have a specific format that enables Agent to implement without interpretation. This reference defines what makes a plan executable vs. vague.

**Key insight:** PLAN.md IS the executable prompt. It contains everything Agent needs to execute the phase, including objective, context references, tasks, verification, success criteria, and output specification.
</overview>

<core_principle>
A plan is Agent-executable when Agent can read the PLAN.md and immediately start implementing without asking clarifying questions.

If Agent has to guess, interpret, or make assumptions - the task is too vague.
</core_principle>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>[what Agent automated]</what-built>
...
<type name="auto">
**Default task type** - Agent executes autonomously.
...
Use for: Everything Agent can do independently (code, tests, builds, file operations).
</type>

<type name="checkpoint:human-action">
**RARELY USED** - Only for actions with NO CLI/API. Agent automates everything possible first.
...
See: references/cli-automation.md for what Agent can automate.

**Execution:** Agent automates everything with CLI/API, stops only for truly unavoidable manual steps.
</type>

<type name="checkpoint:human-verify">
**Human must verify Agent's work** - Visual checks, UX testing.
...
**Execution:** Agent builds the feature, stops, provides testing instructions, waits for approval/feedback.
</type>
...
**Execution:** Agent presents options with balanced pros/cons, waits for decision, proceeds with chosen direction.
</type>

**When to use checkpoints:**
- Visual/UX verification (after Agent builds) → `checkpoint:human-verify`
...
- Anything with CLI/API (Agent automates it) → `type="auto"`
...
**Golden rule:** If Agent CAN automate it, Agent MUST automate it. See: references/cli-automation.md
...
Reference files that Agent needs to understand before implementing.
</context_references>
...
Agent: "How? What type? What library? Where?"
</too_vague>
...
Agent can implement this immediately.
</just_right>

<too_detailed>
Writing the actual code in the plan. Trust Agent to implement from clear instructions.
</too_detailed>
</specificity_levels>
...
These require Agent to decide WHAT to do. Specify it.
</vague_actions>
...
Agent doesn't know your standards. Be explicit.
</missing_context>
</anti_patterns>

<sizing_tasks>
Good task size: 15-60 minutes of Agent work.

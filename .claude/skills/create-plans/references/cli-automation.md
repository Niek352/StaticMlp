...
**Core principle:** If it has a CLI or API, Agent does it. Never ask the human to perform manual steps that Agent can automate.

This reference documents what Agent CAN and SHOULD automate during plan execution.
...
**Critical distinction:** When Agent tries to use a CLI/API and gets an authentication error, this is NOT a failure - it's a gate that requires human input to unblock automation.

**Pattern: Agent encounters auth error → creates checkpoint → you authenticate → Agent continues**
...
<!-- After authentication, Agent retries automatically -->
...
<!-- After key provided, Agent writes to .env and continues -->
...
**When Agent encounters authentication error during execution:**
...
**Key difference:** Agent does EVERYTHING possible first (account creation, API requests), only asks human for the one thing with no automation path.

## Quick Reference: "Can Agent automate this?"

| Action | CLI/API? | Agent does it? |
|--------|----------|-----------------|
...
**Default answer: YES.** Unless explicitly in the "NO" category, Agent automates it.
...
**The rule:** If Agent CAN do it, Agent MUST do it.

Checkpoints are for:
- **Verification** - Confirming Agent's automated work looks/behaves correctly
...
**This keeps the agentic coding workflow intact - Agent does the work, you verify results.**
